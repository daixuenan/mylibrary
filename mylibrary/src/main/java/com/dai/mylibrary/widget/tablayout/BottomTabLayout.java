package com.dai.mylibrary.widget.tablayout;
...
public class BottomTabLayout extends HorizontalScrollView {
    ...
    static final int DEFAULT_GAP_TEXT_ICON = 3; // dps
    ...
    private void addTabFromItemView(@NonNull TabItem item) {
        ...
        if (!TextUtils.isEmpty(item.mSelectedIconUrl)) {
            tab.setSelectedUrl(item.mSelectedIconUrl);
        }
        if (!TextUtils.isEmpty(item.mUnSelectedIconUrl)) {
            tab.setUnSelectedUrl(item.mUnSelectedIconUrl);
        }
    ...
    /**
     * Returns the current mode used by this {@link BottomTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    public static final class Tab {
        ...
        public Tab setIcon(@DrawableRes int resId) {
            if (mParent == null) {
                throw new IllegalArgumentException("Tab not attached to a BottomTabLayout");
            }
    ...
    class TabView extends LinearLayout {
        ...
        private void updateTextAndIcon(@Nullable final TextView textView,
            ...
            final String iconUrl = mTab != null ? mTab.isSelected() ? mTab.getSelectedUrl() : mTab.getUnSelectedUrl() : null;
            ...
            if (iconView != null) {
                if (icon != null) {
                    ...
                    ImageUtils.loadImage(getContext(), iconUrl, iconView);
                    iconView.setVisibility(VISIBLE);
                    setVisibility(VISIBLE);
                } else {
     ...
     * necessary calls back to the provided {@link BottomTabLayout} so that the tab position is
     * kept in sync.
     * <p>
     * <p>This class stores the provided BottomTabLayout weakly, meaning that you can use
